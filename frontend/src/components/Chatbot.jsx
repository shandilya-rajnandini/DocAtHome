...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { from: "bot", text: "Hello! I'm your Doc@Home medical assistant. How can I help you with your healthcare needs today?", timestamp: Date.now() }
  ]);
        ...
        >
          <div className="flex justify-between items-center p-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="font-semibold text-lg">Doc@Home Assistant</span>
            ...
            {messages.map((msg, index) => (
              ...
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${
                  ...
                  {msg.from === "user" ? <User className="w-4 h-4" /> : <Heart className="w-4 h-4" />}
                </div>

                <div className={`flex flex-col max-w-xs ${
                  msg.from === "user" ? "items-end" : "items-start"
                }`}>
                  <div className={`p-3 rounded-2xl text-sm leading-relaxed transition-all duration-200 hover:shadow-md border ${
                    msg.from === "user"
              ...
              <div className="flex items-start space-x-3 animate-in slide-in-from-bottom-3 fade-in duration-300">
                ...
                <div className="bg-white border border-emerald-100 p-3 rounded-2xl rounded-bl-md animate-pulse shadow-sm">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-emerald-600">Analyzing your query...</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="border-t border-gray-100 p-4 bg-white">
            ...
            <div className="text-xs text-gray-500 mt-2 text-center">
              Press Enter to send • Shift+Enter for new line • <span className="text-emerald-600">Confidential & Secure</span>
            </div>
          </div>
        </div>
      )}
