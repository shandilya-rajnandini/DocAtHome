...
const RelayNote = ({ isOpen, onClose, onSubmit, initialValue = "" }) => {
  const [notes, setNotes] = useState(initialValue);

  useEffect(() => {
    if (isOpen) {
      setNotes(initialValue); // Load existing note when modal opens
    } else {
      setNotes(""); // Clear when closed
    }
  }, [isOpen, initialValue]);
