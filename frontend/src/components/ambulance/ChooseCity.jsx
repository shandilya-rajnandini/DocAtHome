...
const ChooseCity = () => {
    return (
        <section className="py-20 px-4 bg-amber-200 dark:bg-primary-dark text-white">
            <div className="container mx-auto text-center">
                <h2 className="text-3xl font-bold text-black dark:text-white">CHOOSE A CITY TO <span className="text-red-500 ">BOOK AN AMBULANCE</span></h2>
                <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                    {cities.map(city => (
                        <div key={city.name} className="group cursor-pointer">
                            ...
                            <p className="mt-2 font-semibold text-black dark:text-white">{city.name}</p>
                        </div>
                    ))}
                </div>
            </div>
