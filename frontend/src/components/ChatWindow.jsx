    ...
    <div className="fixed bottom-4 right-4 w-96 h-[500px] bg-secondary-dark rounded-lg shadow-2xl flex flex-col z-50">
      ...
      <div className="flex-grow p-4 overflow-y-auto">
        {messageList.map((msg, index) => (
          <div
            key={index}
            className={`mb-3 flex ${msg.author === user.name ? "justify-end" : "justify-start"}`}
          >
            <div
              className={`p-3 rounded-lg max-w-[70%] ${msg.author === user.name ? "bg-accent-blue text-white" : "bg-gray-700"}`}
            >
              <p>{msg.message}</p>
      ...
      <div className="p-3 border-t border-gray-700 flex">
        ...
        <button
          onClick={sendMessage}
          className="bg-accent-blue text-white px-4 rounded-r-md"
        >
          Send
        </button>
      </div>
