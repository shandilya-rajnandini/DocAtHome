  ...
  const findNearbyNurses = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        toast.success(
          `Found nurses within ${filters.radius || "10"}km of your location!`,
        );
      },
      (error) => {
        setIsGettingLocation(false);
      ...
      <div className="relative bg-[url('/nurse-header-bg.jpg')] bg-cover bg-center h-60 md:h-80">
        <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col justify-center items-center text-center p-4">
          ...
          <p className="text-lg md:text-xl text-gray-300 mt-4">
            Book skilled and compassionate nurses for personalized home care.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="lg:col-span-1 bg-secondary-dark p-6 rounded-lg shadow-lg h-fit">
          ...
          <button
            ...
            className="w-full bg-accent-blue text-white p-3 rounded font-bold hover:bg-accent-blue-hover transition"
          >
            Search Nurses
          </button>
        </aside>
...
const NurseCard = ({ nurse }) => (
  <div className="bg-secondary-dark p-6 rounded-lg shadow-lg flex gap-6">
    <img
      ...
      alt={nurse.name}
      className="w-32 h-32 rounded-full object-cover border-4 border-gray-700 flex-shrink-0"
    />
    <div className="flex-grow">
      ...
      <div className="flex items-center gap-4 my-2 text-sm">
        <span className="bg-gray-700 px-3 py-1 rounded-full">
          {nurse.experience} Yrs Experience
        </span>
        {nurse.averageRating && (
          <span className="text-yellow-400">
            {nurse.averageRating.toFixed(1)} ★ ({nurse.numReviews || 0} reviews)
          </span>
        )}
      </div>
    ...
    <div className="flex items-center">
      <Link
        to={`/nurses/${nurse._id}`}
        className="bg-teal-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-teal-700 transition"
      >
        View & Book
      </Link>
    </div>
