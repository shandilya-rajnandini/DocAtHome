...
const SearchDoctorsPage = () => {
    ...
    // Fetch initial doctors on component mount
    useEffect(() => {
        fetchDoctors({});
    }, []);
    ...
    const handleSpecialtyInput = (e) => {
        ...
        const filteredSuggestions = doctorSpecialties.filter(spec =>
            spec.toLowerCase().includes(value.toLowerCase())
        );
        setSpecialtySuggestions(filteredSuggestions);
        
        setFilters(prevFilters => ({ ...prevFilters, specialty: value }));
    };
    ...
    const handleCityInput = (e) => {
        ...
        setFilters(prevFilters => ({ ...prevFilters, city: value }));
    };
    
    const handleSpecialtySelect = (specialty) => {
        setSpecialtyInput(specialty);
        setFilters(prevFilters => ({ ...prevFilters, specialty }));
    ...
    // --- HANDLER TO CLOSE DROPDOWNS ON OUTSIDE CLICK ---
    useEffect(() => {
    ...
    // Handles changes to regular select filters (experience, sort by, etc.)
    ...
    const handlePageChange = (page) => {
        if (page > 0 && page <= pagination.totalPages) {
            fetchDoctors({ ...filters, page: page });
        }
    };
        ...
        <div>
            ...
            <div className="container mx-auto p-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <form onSubmit={handleApplyFilters} className="lg:col-span-1 bg-white dark:bg-secondary-dark p-6 rounded-lg shadow-lg h-fit">
                    ...
                    <div className="mb-6 relative" ref={specialtyRef}>
                        ...
                        {showSpecialtyDropdown && specialtySuggestions.length > 0 && (
                            <ul className="absolute z-10 w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg mt-1 max-h-60 overflow-y-auto">
                                {specialtySuggestions.map((spec, index) => (
                                    <li 
                                        key={index} 
                                        onClick={() => handleSpecialtySelect(spec)}
                                        className="p-3 cursor-pointer text-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600"
                                    >
                                        {spec}
                                    </li>
                                ))}
                    ...
                    {/* --- CITY AUTOCOMPLETE --- */}
                    <div className="mb-6 relative" ref={cityRef}>
