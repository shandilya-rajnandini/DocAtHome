...
const NurseProfilePage = () => {
  ...
  const handleBookAssignment = async () => {
    ...
    if (!bookingDetails.symptoms)
      return toast.error("Please describe the care required.");
    ...
    try {
      ...
      toast.error(error.response?.data?.msg || "Failed to book assignment.", {
        id: toastId,
      });
    }
  };
  ...
  if (!nurse)
    return (
      <div className="text-center p-10 text-red-500">Nurse not found.</div>
    );
    ...
    return (
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-secondary-dark p-6 rounded-lg shadow-lg">
            ...
            <div className="flex space-x-2 overflow-x-auto pb-4">
              {availableDates.map((date) => (
                <button
                  ...
                  className={`p-4 rounded-lg text-center w-20 flex-shrink-0 ${selectedDate === date.fullDate ? "bg-teal-500 text-white" : "bg-primary-dark text-secondary-text"}`}
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
              {timeSlots.map((time) => (
                <button
                  ...
                  className={`p-3 rounded-lg ${selectedTime === time ? "bg-teal-500 text-white" : "bg-primary-dark text-secondary-text border border-gray-700"}`}
                >
                  {time}
                </button>
              ))}
            ...
            <div className="mt-8 border-t border-gray-700 pt-6">
              ...
              <textarea
                name="symptoms"
                onChange={(e) =>
                  setBookingDetails({ symptoms: e.target.value })
                }
                placeholder="e.g., Post-operative wound care, vitals monitoring..."
                rows="4"
          ...
          <div className="lg:col-span-1 bg-secondary-dark p-6 rounded-lg shadow-lg h-fit">
            ...
            <button
              onClick={handleBookAssignment}
              className="w-full bg-green-600 text-white p-3 mt-6 rounded font-bold hover:bg-green-700 transition"
            >
              Confirm & Book Nurse
            </button>
          </div>
