...
const ProBenefitCard = ({ icon, title, description, isActive }) => (
  <div
    className={`p-4 rounded-lg border-2 ${isActive ? "border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20" : "border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800"}`}
  >
    <div className="flex items-start gap-3">
      <span className="text-2xl">{icon}</span>
      <div>
        <h4
          className={`font-semibold ${isActive ? "text-yellow-700 dark:text-yellow-300" : "text-gray-600 dark:text-gray-300"}`}
        >
          {title}
        </h4>
        <p
          className={`text-sm ${isActive ? "text-yellow-600 dark:text-yellow-400" : "text-gray-500 dark:text-gray-400"}`}
        >
          {description}
        </p>
        {isActive && (
...
const NurseDashboard = () => {
  ...
  const isSubscriptionActive = () => {
    const hasProTier =
      user?.subscriptionTier === "pro" || subscriptionStatus?.tier === "pro";
        ...
        <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col justify-center items-start p-8 md:p-12">
          ...
          {isPro && subscriptionStatus?.expiry && (
            <p className="text-sm text-yellow-400 mt-1">
              ...
              {new Date(subscriptionStatus.expiry).toLocaleDateString()}
            </p>
          )}
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between">
              <div className="text-white mb-4 md:mb-0">
                ...
                <p className="text-teal-100">
                  Get higher visibility, advanced analytics, and lower fees!
                </p>
              </div>
              <Link
                to="/upgrade-pro"
        ...
        {isPro && (
            ...
            <div className="bg-secondary-dark p-6 rounded-lg shadow-lg">
              ...
              <div className="space-y-3">
                <Link
                  ...
                  className="block w-full text-center bg-teal-600 text-white p-3 rounded font-bold hover:bg-teal-700 transition"
                >
                  Manage Assignments
                </Link>
                <Link
                  to="/nurse/availability"
                  className="block w-full text-center bg-gray-700 text-white p-3 rounded font-bold hover:bg-gray-600 transition"
                >
                  Update Availability
                </Link>
                <Link
                  to="/nurse/edit-profile"
                  className="block w-full text-center bg-gray-700 text-white p-3 rounded font-bold hover:bg-gray-600 transition"
                >
                  Edit My Profile
                </Link>
                {isPro && (
                  <Link
                    to="/demand-hotspot"
                    className="block w-full text-center bg-gradient-to-r from-yellow-500 to-yellow-600 text-black p-3 rounded font-bold hover:from-yellow-600 hover:to-yellow-700 transition"
                  >
                    📊 Demand Hotspot Map
                  </Link>
                )}
                {!isPro && ["doctor", "nurse"].includes(user?.role) && (
                  <Link
                    to="/upgrade-pro"
                    className="block w-full text-center bg-gradient-to-r from-purple-600 to-blue-600 text-white p-3 rounded font-bold hover:from-purple-700 hover:to-blue-700 transition"
                  >
                    ⭐ Upgrade to Pro
                  </Link>
                )}
          ...
          <div className="lg:col-span-2 bg-secondary-dark p-6 rounded-lg shadow-lg">
            ...
            <div className="text-center text-secondary-text py-20 border-2 border-dashed border-gray-700 rounded-lg">
              ...
              <p className="mt-2">
                Your available slots will be visible to patients.
              </p>
            </div>
          </div>
        </div>
