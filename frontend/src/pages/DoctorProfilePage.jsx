...
const DoctorProfilePage = () => {
  ...
  const [shareRelayNote, setShareRelayNote] = useState(false);
...
useEffect(() => {
  ...
  // Handle query params for booking details
...
<div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
  <div className="lg:col-span-2 bg-secondary-dark p-6 rounded-lg shadow-lg">
    ...
    <div className="flex space-x-2 overflow-x-auto pb-4">
      {availableDates.map((date) => (
        <button
          ...
          className={`p-4 rounded-lg text-center w-20 flex-shrink-0 ${
            selectedDate === date.fullDate
              ? "bg-accent-blue text-white"
              : "bg-primary-dark text-secondary-text"
          }`}
        >
      ...
      {timeSlots.map((time) => (
        <button
          ...
          className={`p-3 rounded-lg ${
            selectedTime === time
              ? "bg-accent-blue text-white"
              : "bg-primary-dark text-secondary-text border border-gray-700"
          }`}
        >
          {time}
        </button>
    ...
    <div className="mt-8 border-t border-gray-700 pt-6">
      ...
      <div className="space-y-4">
        ...
        <input
          type="text"
          name="previousMeds"
          value={bookingDetails.previousMeds}
          onChange={handleBookingDetailChange}
          placeholder="List any previous medications (optional)"
          className="w-full p-3 bg-primary-dark rounded-md border-gray-700 text-white"
        />
        <div>
          <label className="block text-secondary-text mb-2">
            Upload previous report/photo (optional)
          </label>
          <input
            type="file"
            name="reportImage"
            onChange={handleBookingDetailChange}
            className="w-full text-secondary-text file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-accent-blue file:text-white hover:file:bg-accent-blue-hover"
          />
        </div>
      </div>
    </div>
    <label className="flex items-center gap-2 mt-4">
      <input
        type="checkbox"
        checked={shareRelayNote}
        onChange={(e) => setShareRelayNote(e.target.checked)}
        className="form-checkbox"
      />
      Securely share the note from my last consultation with this professional.
    </label>
  </div>
  ...
  <div className="lg:col-span-1 bg-secondary-dark p-6 rounded-lg shadow-lg h-fit">
    ...
    <div className="mt-6">
      ...
      <div className="flex rounded-lg border border-gray-700">
        <button
          onClick={() => setBookingType("In-Home Visit")}
        ...
        <button
          ...
          className={`w-1/2 p-3 rounded-r-lg ${
    ...
    <button
      ...
      className="w-full bg-green-600 text-white p-3 mt-6 rounded font-bold hover:bg-green-700 transition duration-300"
    >
      Confirm & Book Appointment
            ...
            <label className="flex items-center gap-2 mt-4">
              <input
                ...
                className="form-checkbox"
              />
              Securely share the note from my last consultation with this
              professional.
            </label>
          </div>
