...
const CareCirclePage = () => {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      ...
      toast.error(
        "Failed to send invite. Ensure the user has a Doc@Home account.",
      );
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-accent-cream dark:bg-secondary-dark p-6 rounded-lg shadow-lg h-fit">
            ...
            <p className="text-secondary-text text-sm mb-4">
              Add family members, doctors, or nurses to your circle to
              collaboratively manage your health.
            </p>
            <form onSubmit={handleInvite} className="space-y-4">
              <div>
                <label className="block  text-black dark:text-secondary-text  mb-1">
                  Email Address
                </label>
                <input
                  type="email"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-3 bg-gray-300 dark:bg-primary-dark rounded-md border-gray-700 text-black dark:text-white"
                >
              ...
              <button
                type="submit"
                className="w-full bg-accent-blue text-white font-bold py-3 px-6 rounded-lg hover:bg-accent-blue-hover"
              >
                Send Invite
              </button>
            </form>
              ...
              <div className="space-y-3">
                {circle?.members.length > 0 ? (
                  circle.members.map((member) => (
                    <div
                      key={member._id}
                      className="flex items-center justify-between bg-primary-dark p-3 rounded-md"
                    >
                      <div className="flex items-center">
                        <div className="text-2xl mr-4 w-8 text-center">
                      ...
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-full ${member.status === "Active" ? "bg-green-800 text-green-300" : "bg-yellow-800 text-yellow-300"}`}
                      >
                        {member.status}
                      </span>
                    </div>
                  ...
                  <p className="text-secondary-text text-center py-4">
                    Your circle is empty. Use the form to invite someone!
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-accent-cream dark:bg-secondary-dark p-6 rounded-lg shadow-lg">
              ...
              <div className="text-center text-black dark:text-secondary-text py-16 border-2 border-dashed border-gray-700 rounded-lg">
                ...
                <p className="mt-2 text-sm">
                  (A chart visualizing health trends will be available soon)
                </p>
              </div>
            </div>
          </div>
