...
const BookLabTestPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(
        "Lab test booked successfully! Our partner lab will contact you shortly.",
      );
    ...
    <div>
      ...
      <div className="container mx-auto p-8">
        <form
          ...
          className="max-w-4xl mx-auto bg-amber-50 dark:bg-secondary-dark  p-8 rounded-lg shadow-lg"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Column 1: Test & Address Details */}
            <div className="space-y-6">
              <div>
                <label className="block text-black dark:text-secondary-text mb-2 font-semibold">
                  Test Name*
                </label>
                <input
                  type="text"
                  ...
                  onChange={onChange}
                  placeholder="e.g., Complete Blood Count (CBC), Thyroid Profile"
                  className="w-full p-3 bg-gray-200 dark:bg-primary-dark text-black dark:text-white rounded-md border-gray-700 "
                />
              </div>
              <div>
                <label className="block text-black dark:text-secondary-text mb-2 font-semibold">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  type="date"
                  name="collectionDate"
                  value={bookingData.collectionDate}
                  onChange={onChange}
                  className="w-full p-3 bg-gray-200 dark:bg-primary-dark text-black dark:text-white rounded-md border-gray-700 "
                />
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {timeSlots.map((time) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setBookingData({ ...bookingData, collectionTime: time })
                      }
          ...
          <div className="border-t border-gray-700 mt-8 pt-6 flex flex-col items-center">
            <div className="text-center">
              <p className="text-black dark:text-secondary-text">
                Total Payable Amount
              </p>
              <p className="text-4xl font-bold text-accent-blue mt-1">₹800</p>
              <p className="text-xs text-black dark:text-secondary-text">
                (Includes Visit & Transportation Charges)
              </p>
            </div>
            <button
              type="submit"
              className="w-full md:w-1/2 mt-6 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg transition"
            >
              Confirm & Book Test
            </button>
          </div>
